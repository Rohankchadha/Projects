...
class Frame23:    
    ...
    def getdata(self):
        ...
        if a!='' and b!='' and c!='' and d!='':
            data3={'Hobby - ':a,'Sports - ':b,'Achievements - ':c,'Extra - ':d}
            for i in range(0,len(self.var1)):
                if self.var1[i].get()!='' and self.var[i].get()!='':
                    data3.update({self.var1[i].get()+' - ':self.var[i].get()})
                else:
                    messagebox.showwarning('Attention', 'All Fields are mandatory')
                    break
            Frame3.Frame3.data.append(data3)
            self.fr23.destroy()
            obj=Frame3.Frame3()
            obj.create3()
        else:
    ...
    def create23(self):
        ...
        #     self.a=file.read()
        self.fr23 = Frame(self.root)
        self.fr23.place(x=0, y=0, width=1000, height=650)
        self.fr23.config(bg="black")
        ...
        self.btn2 = Button(self.fr23, text='Add no. of new field', command=self.addnew)
        self.btn2.place(x=700, y=550, width=200, height=30)

        self.btn=Button(self.fr23,text='Next >', command=lambda :self.getdata())
        self.btn.place(x=600, y=600, width=300, height=50)

        self.btn1=Button(self.fr23,text='Skip ^', command=lambda :self.skip())
