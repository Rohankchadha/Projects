...
class Frame21:    
    ...
    def getdata(self):
        ...
        if a!='' and b!='' and c!='' and d!='':
            data1={'School - ':a,'High School - ':b,'University - ':c,'Extra - ':d}
            for i in range(0,len(self.var1)):
                if self.var1[i].get()!='' and self.var[i].get()!='':
                    data1.update({self.var1[i].get()+' - ':self.var[i].get()})
                else:
                    messagebox.showwarning('Attention', 'All Fields are mandatory')
                    break
            # print(data)
            Frame3.Frame3.data.append(data1)
            self.fr21.destroy()
            obj=Frame22.Frame22(self.root)
            obj.create22()
        else:
    ...
    def create21(self):
        ...
        self.fr21.place(x=0, y=0, width=1000, height=650)
        ...
        self.btn2 = Button(self.fr21, text='Add no. of new field', command=self.addnew)
        self.btn2.place(x=700, y=550, width=200, height=30)

        self.btn=Button(self.fr21,text='Next >', command=lambda :self.getdata())
        self.btn.place(x=600, y=600, width=300, height=50)

        self.btn1=Button(self.fr21,text='Skip ^', command=lambda :self.skip())
