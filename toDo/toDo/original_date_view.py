...
class daily():
    ...
    def daily_task(self,data):
        ...
        value = (self.data[0],today_date)
        for i in dataBase.view_today(value):
            print(i)
    ...
    def maindata(self,e):
        print("jallazlkm",self.combodate.get(),"nlk")
        for item in self.tr.get_children():
            self.tr.delete(item)

        value = (self.data[0],self.combodate.get())
        print(value)
        if self.combodate.get()!="":
            val = self.data[0],self.combodate.get()
            res = dataBase.view_today(val)
            for i in res:
                print(res)
                self.tr.insert('', 0, text = i[0], values=(i[2],i[3],i[4],i[5],i[6],i[7],"Edit","Delete"),tags=(i[6]))
